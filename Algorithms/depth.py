        ...
        for action in ((0, 1), (0, -1), (1, 0), (-1, 0)):
            ...
            if 0 <= x < len(self.world) and 0 <= y < len(self.world[0]):
                ...
                if self.world[x][y] != 1:
                    ...
                    # Poner un 0 para el paso anterior si era la posición actualo la nave
                    new_state[old_x][old_y] = 0 if self.world[old_x][old_y] == 2 or self.world[old_x][old_y] == 3 else self.world[old_x][old_y]
                    # Posición actual
                    new_state[x][y] = 2
...
class CustomPriorityQueue:
    ...
    def push(self, current_node):
        tmp_node = CustomNode(curr_depth=current_node.depth, curr_node=current_node)
        heapq.heappush(self.queue, (tmp_node.current_depth, tmp_node))
...
class CustomNode:
    def __init__(self, curr_depth, curr_node):
        self.current_depth = curr_depth
        self.current_node = curr_node
    def __lt__(self, other):
        return self.current_depth < other.current_depth