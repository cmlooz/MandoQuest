        ...
        for action in ((0, 1), (0, -1), (1, 0), (-1, 0)):
            ...
            if 0 <= x < len(self.world) and 0 <= y < len(self.world[0]):
                ...
                if self.world[x][y] != 1:
                    ...
                    new_state[old_x][old_y] = 0 if self.world[old_x][old_y] == 2 or self.world[old_x][old_y] == 3 else 3 if curr_node.has_ship == self.max_ship_fuel and self.world[old_x][old_y] != 3 else self.world[old_x][old_y]
                    # Posición actual
                    new_state[x][y] = 2
...
class CustomPriorityQueue:
    ...
    def push(self, current_node):
        tmp_node = CustomNode(curr_cost=current_node.current_cost, curr_heuristic=current_node.heuristic , curr_node=current_node)
        heapq.heappush(self.queue, (tmp_node.current_cost, tmp_node))
...
class CustomNode:
    def __init__(self, curr_cost, curr_heuristic, curr_node):
        self.current_cost = curr_cost
        self.current_heuristic = curr_heuristic
        self.current_node = curr_node
    def __lt__(self, other):
        return self.current_cost + self.current_heuristic < other.current_cost + other.current_node.heuristic